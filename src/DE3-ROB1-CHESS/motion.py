...
if __name__ == '__main__':
    ...
    planner = MotionPlanner(visual=True, debug=True)
    # path = planner.apply_trapezoid_vel([[0,0,0], [1,2,1]), [2,2,2]])
    # planner.smooth_corners(path, 2, 10)
    # EXAMPLE MOTION TO TEST IF PARTS ARE WORKING
    # example_path = np.array([[arm.x, arm.y, arm.z], [arm.x, arm.y, arm.z-0.3]])
    # motion_plan = planner.apply_trapezoid_vel(example_path)
    # arm.send_trajectory(motion_plan)
    arm.grasp(0.1, 0.5, 60)


    # example_path = np.array([[arm.x, arm.y, arm.z-0.3], [arm.x, arm.y, arm.z]])
    # motion_plan = planner.apply_trapezoid_vel(example_path)
    # arm.send_trajectory(motion_plan)
    # arm.move_gripper(0.8, 10)
